...
public class HeadLayerFeatureRenderer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    private Set<Item> hideHeadLayers = Sets.newHashSet(Items.ZOMBIE_HEAD, Items.CREEPER_HEAD, Items.DRAGON_HEAD,
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, AbstractClientPlayer player,
        ...
        if (itemStack != null && itemStack.getItem() == Items.PLAYER_HEAD
                && !SkinLayersModBase.disguiseHeadsCompatibility) {
            return;
        }
    ...
    public void renderCustomHelmet(PlayerSettings settings, AbstractClientPlayer abstractClientPlayer,
            PoseStack matrixStack, VertexConsumer vertices, int light, int overlay) {
        if (settings.getHeadMesh() == null)
            return;
        if (!this.getParentModel().head.visible || !abstractClientPlayer.isModelPartShown(PlayerModelPart.HAT))
            return;
        ...
        settings.getHeadMesh().render(this.getParentModel().head, matrixStack, vertices, light, overlay, 1.0f, 1.0f,
                1.0f, 1.0f);
        matrixStack.popPose();
